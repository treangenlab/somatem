...
open(my $file, '<', $asn) || die "\n Error: The ASN file $asn cannot be found\n\n"; 
my $firstLine = <$file>; ## Grab the first line from the ASN file
if ($firstLine !~ m/^Blast4-archive/) { ## First line has to start with 'Blast4-archive' for ASN.1 format
    die "\n Error the ASN file provided ($asn) does not look to be in ASN.1 format. I.e. does not have a leading 'Blast4-archive' in the first line.\n\n";
}
close($file);
...
foreach my $i (@Formats) {
	...
	$blast_exe = "blast_formatter -archive $asn -out $output_file -outfmt $i";
    }
