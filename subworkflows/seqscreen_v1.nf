...
/* SeqScreen branch that composes the repo's .nf workflows:
...
This subworkflow integrates the existing DSL2 SeqScreen workflows.
...
Or with the config file approach:
nextflow run seqscreen.nf -c nextflow.config \
    --fasta /path/to/sequences.fasta \
    --databases /path/to/seqscreen/databases \
    --workflows_dir /path/to/seqscreen/workflows \
    --bin_dir /path/to/seqscreen/bin \
...
workflow {
    ...
    if (!params.fasta) {
        error "Please provide a FASTA file with --fasta"
    }
    if (!params.databases) {
    ...
    // Create input channel from FASTA file(s)
    // Support both single file and glob patterns
    ch_input = Channel
    ...
    SEQSCREEN_BRANCH.out.report_html_ch.view { meta, html -> "HTML report for ${meta.id}: ${html}" }
    SEQSCREEN_BRANCH.out.tables_ch.view { meta, table -> "Table report for ${meta.id}: ${table}" }
}